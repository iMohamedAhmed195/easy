...
class MoneyAndOilProviderBottomNavigationWidget extends StatelessWidget {
  ...
  final List<TitleIconModel> moneyProviderAndOilNavBar = [
    ...
    TitleIconModel(
        ...
        iconDisabled: AppAssets.qrCode,
        iconActive: AppAssets.qrCodeActive),
  ];
  ...
  Widget build(BuildContext context) {
    return BottomNavigationBar(
      selectedLabelStyle: getBoldTextStyle(
          fontSize: 10, height: 1.7, color: AppColors.primaryColor),
      selectedIconTheme: const IconThemeData(color: AppColors.primaryColor),
      unselectedLabelStyle:
          getBoldTextStyle(fontSize: 10, color: AppColors.gray),
      ...
      items: List.generate(
        ...
        (index) => BottomNavigationBarItem(
          icon: context.read<BottomNavOperationCubit>().index == index
              ? CustomSVGImage(
                  asset: moneyProviderAndOilNavBar[index].iconActive)
              : CustomSVGImage(
